...
import com.catpuppyapp.sshkeyman.utils.addPrefix
...
fun ChangeListTitle(
    ...
    dropDownMenuItemOnClick: (SshKeyEntity) -> Unit,
...
//                    .heightIn(max=MyStyleKt.Title.lineHeight)  //弃用，因为会使字体上下部分都显示不全！
                    ...
                    Text(
                        text = changeListCurRepo.value.name,
...
//            }
                ...
                //如果是当前条目，名字前加个星号
                val repoNameMayHavePrefix = if(r.name == changeListCurRepo.value.name) addPrefix(r.name) else r.name
                //列出其余仓库
                DropdownMenuItem(
                    text = { Text(repoNameMayHavePrefix) },
                    onClick = {
                        //如果点击其他仓库(切换仓库)，则退出选择模式
                        if(changeListCurRepo.value.name != r.name) {
    ...
    LaunchedEffect(needReQueryRepoList.value) {
        try {
            doJobThenOffLoading {
                val repoDb = AppModel.singleInstanceHolder.dbContainer.sshKeyRepository
