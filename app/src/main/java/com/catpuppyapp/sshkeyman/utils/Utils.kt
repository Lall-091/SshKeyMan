...
import com.catpuppyapp.sshkeyman.etc.Ret
import com.catpuppyapp.sshkeyman.utils.encrypt.PassEncryptHelper
...
fun getFilePathStrBasedAllRepoDir(path:String):String {
    ...
    val allRepoBaseDirParentFullPath = AppModel.singleInstanceHolder.homeDir.parent?:""  //不知道parent是否以/结尾，如果是，后面解析出的内容会是 非/开头，直接返回即可，否则会是/开头，需要删除开头的/再返回
    ...
    if(filePathStrBasedAllRepoDir.isNotBlank()) {
        ...
        if(cutAllRepoDirIndex!=0 && cutAllRepoDirIndex < filePathStrBasedAllRepoDir.length) {  //索引加了1，如果还是0，说明没找到对应字符串
            ...
            if(finallyStrIndex!=0 && finallyStrIndex < repoPathUnderAllRepoBase.length) {
                ...
                repoFullPath = File(AppModel.singleInstanceHolder.homeDir.canonicalPath, repoNameEndsWithSeparator).canonicalPath  //canonicalPath 返回的结果末尾就没 / 了，结果应为 Repo
...
fun<T> doActIfIndexGood(idx:Int, list:List<T>, act:(T)-> Unit):Ret<T?> {
    try {
        ...
        return Ret.createError(null, "err:invalid index for list", Ret.RetCode.err_invalidIdxForList)
    }catch (e:Exception) {
        MyLog.e(TAG, "#doActIfIndexGood() err:"+e.stackTraceToString())
        return Ret.createError(null, "err:"+e.localizedMessage, Ret.RetCode.err_doActForItemErr)
    }
...
fun copyAndShowCopied(text:String, context: Context, clipboardManager: ClipboardManager) {
    clipboardManager.setText(AnnotatedString(text))
    showToast(context = context, context.getString(R.string.copied), duration = Toast.LENGTH_SHORT)
}


fun encryptIfNotEmpty(content:String):String {
    //用户名不用加密，不过私钥呢？感觉也用不着加密，暂时只加密密码吧。
    if(content.isNotEmpty()) {
        return PassEncryptHelper.encryptWithCurrentEncryptor(content)
    }else {
        return content
    }
}

fun decryptIfNotEmpty(content:String):String {
    if (content.isNotEmpty()) {
        //如果密码不为空，解密密码。
        return PassEncryptHelper.decryptWithCurrentEncryptor(content)
    }else {
        return content
    }
}
